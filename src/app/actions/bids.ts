...
export async function acceptBid(formData: FormData): Promise<void> {
  ...
  const admin = createAdminClient();
  const feeResolution = await resolvePlatformFeeForBidAcceptance(admin, {
    contractorId: bid.contractor_id,
    customerId: user.id,
  });
  const {
  ...
  if (feeWaived && invoiceRes.invoiceId) {
    ...
    if (appliedCustomerReferral && customerReferralCredit && invoiceRes.invoiceId) {
  ...
  scheduleNotification(() =>
    userNotifyBidAccepted({
      ...
      feeCents,
      feeWaiverReason: waiverReason,
      customerReferralDiscountCents: appliedCustomerReferral
        ? customerReferralDiscountCents
        : 0,
      acceptedAmountCents,
