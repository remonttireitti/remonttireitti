-- Asiakkaan suosittelu (ajetaan tuotannossa kerran).
...
DROP POLICY IF EXISTS "customer_referrals: admin read" ON public.customer_referrals;
...
DROP POLICY IF EXISTS "customer_referrals: referrer read own" ON public.customer_referrals;
CREATE POLICY "customer_referrals: referrer read own"
  ON public.customer_referrals FOR SELECT
  USING (referrer_customer_id = auth.uid());

DROP POLICY IF EXISTS "customer_referral_credits: owner read" ON public.customer_referral_credits;
CREATE POLICY "customer_referral_credits: owner read"
  ON public.customer_referral_credits FOR SELECT
  USING (customer_id = auth.uid());

DROP POLICY IF EXISTS "customer_referral_credits: admin read" ON public.customer_referral_credits;
