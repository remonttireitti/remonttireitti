...
private:
    ...
    int ensure_connection() {
        if (persistent_sock_ >= 0) {
            char probe;
            int r = ::recv(persistent_sock_, &probe, 1, MSG_PEEK | MSG_DONTWAIT);
            if (r == 0) {
                close_socket();
        ...
        persistent_sock_ = create_connection();
        return persistent_sock_;
    }

    bool read_exact(int sock, uint8_t *buf, size_t len) {
        size_t got = 0;
        while (got < len) {
    ...
    int create_connection() {
        ...
        struct timeval timeout;
        timeout.tv_sec = 3;
        timeout.tv_usec = 0;  // 3s read/write timeout (same as HA)
        ...
        if (connect_result < 0) {
            if (errno == EINPROGRESS) {
                ...
                connect_timeout.tv_usec = 0;  // 3s connect timeout (same as HA)
                
                int select_result = ::select(sock + 1, nullptr, &write_fds, nullptr, &connect_timeout);
                if (select_result <= 0) {
                    ESP_LOGW(TAG, "Connection timeout to %s:%d", host_.c_str(), port_);
                    ::close(sock);
                    return -1;
                }
                ...
                if (error != 0) {
                    ESP_LOGW(TAG, "Connection failed to %s:%d (error: %d)", host_.c_str(), port_, error);
                    ::close(sock);
                    return -1;
                }
            } else {
                ESP_LOGW(TAG, "Immediate connection failure to %s:%d (errno %d)", host_.c_str(), port_, errno);
                ::close(sock);
                return -1;
            }
        }
    ...
    std::vector<uint8_t> receive_data(int sock) {
        uint8_t header[6];
        if (!read_exact(sock, header, 6)) {
            return {};
        }
        uint16_t len = (static_cast<uint16_t>(header[4]) << 8) | header[5];
        if (len < 2 || len > 260) {
            return {};
        }
        std::vector<uint8_t> data(6 + len);
        memcpy(data.data(), header, 6);
        if (!read_exact(sock, data.data() + 6, len)) {
            return {};
        }
        return data;
    }
